...
#include "ui_priorite_req_dialog.h"
#include "QTableView"
#include "QVBoxLayout"
...
priorite_req_dialog::priorite_req_dialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::priorite_req_dialog)
{
    ui->setupUi(this);
    ...
    priorite_req ptreq;
    ptreq.affectList();
    ptreq.traiterProcessus();
    float tme = ptreq.getTme();
    float tma = ptreq.getTma();

    QString montme = QString::number(tme);
    QString montma = QString::number(tma);
    modele1 = new QStandardItemModel(1,1);
    modele1->setItem(0, 0, new QStandardItem("PRIORITE AVEC REQUISITION"));
...
priorite_req_dialog::~priorite_req_dialog()
{
    delete ui;
}

void priorite_req_dialog::dessiner(){
    int nbrproc = l.listProcessSize();
    ...
    while (!l.estVide()){
        ...
        while (l.existe()){
            ...
            //__-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_ E/S-__-_-_-_-CPU_--_-_-
...
void priorite_req_dialog::showEvent(QShowEvent *event) {
